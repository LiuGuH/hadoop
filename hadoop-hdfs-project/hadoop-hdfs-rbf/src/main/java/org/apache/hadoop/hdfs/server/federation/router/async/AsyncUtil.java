...
public final class AsyncUtil {
  ...
  public static void asyncThrowException(Throwable e) {
    ...
    result.completeExceptionally(wrapCompletionException(e));
   ...
   * handled using the {@link #asyncCatch(Class, CatchFunction)} method,
   ...
   * @see AsyncRun
   * @see #asyncCatch(Class, CatchFunction)
   */
   ...
   *
   * @param catchFunction The {@link CatchFunction} that defines how to
   ...
   * @see CatchFunction
   */
  public static <R, E extends Throwable> void asyncCatch(Class<E> eClass,
      CatchFunction<R, E> catchFunction) {
    CompletableFuture<R> completableFuture =
        (CompletableFuture<R>) CUR_COMPLETABLE_FUTURE.get();
    assert completableFuture != null;
    CompletableFuture<R> result = catchFunction.apply(completableFuture, eClass);
    CUR_COMPLETABLE_FUTURE.set((CompletableFuture<Object>) result);
  }
  ...
  /**
   * Encapsulate the asynchronous implementation of try-catch-finally statement.
   * @param tryFunction asyncTry
   * @param exceptionClass The Class of Exception may throwned in asyncTry
   * @param catchFunction asyncCatch
   * @param finallyFunction asyncFinally
   */
  public static <T, E extends Throwable, C, F> void asyncTryCatchFinally(
      AsyncRun<T> tryFunction,
      Class<E> exceptionClass,
      CatchFunction<C, E> catchFunction,
      FinallyFunction<F> finallyFunction) {

    if (tryFunction != null) {
      asyncTry(tryFunction);
    }

    if (catchFunction != null) {
      asyncCatch(exceptionClass, catchFunction);
    }

    if (finallyFunction != null) {
      asyncFinally(finallyFunction);
    }
  }
   ...
   * @see CompletableFuture
   */
  public static <I, R, P> void asyncConcurrent(
