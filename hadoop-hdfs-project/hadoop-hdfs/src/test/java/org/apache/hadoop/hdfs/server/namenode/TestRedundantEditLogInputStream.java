...
public class TestRedundantEditLogInputStream {
  ...
  public void testNextOp() throws IOException {
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      when(list.get(i).getName()).thenReturn(FAKE_EDIT_STREAM_NAME + i);
      when(list.get(i).getFirstTxId()).thenReturn(1L);
      when(list.get(i).getLastTxId()).thenReturn(2L);
      when(list.get(i).length()).thenReturn(1L);
    }
