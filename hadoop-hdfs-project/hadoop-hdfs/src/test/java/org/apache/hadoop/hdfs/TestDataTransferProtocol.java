  ...
  public void testCopyBlockCrossNamespace()
    ...
    try {
      ...
      for (DataNode datanode : dataNodes) {
        String dn = datanode.getDatanodeHostname() + ":" + datanode.getXferPort();
        if (!dn.equals(ns0BlockLocation1) && !dn.equals(ns0BlockLocation2)) {
          favoredNodes[1] = dn;
        }
      }
      ...
      for (DataNode dataNode : dataNodes) {
        if (dataNode.getXferPort() == datanodeInfoWithStoragesNS0[0].getXferPort()) {
          assertEquals(1l, dataNode.getMetrics().getFastCopyBlocksReplicatedViaHardlinkSuccesses());
        } else if (dataNode.getXferPort() == datanodeInfoWithStoragesNS0[1].getXferPort()) {
          assertEquals(1l, dataNode.getMetrics().getFastCopyBlocksReplicatedViaTransferSuccesses());
        }
      }
