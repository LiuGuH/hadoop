 ...
 * 
 * Notice: We don't use CacheManager feature in our BZL production envirnment.
 * So, we delete some codes in removeStoredBlocks and processCachedBlocks method
 * and so on.
 * 
 */
...
public class CacheManager {
  ...
  public GSet<CachedBlock, CachedBlock> getCachedBlocks() {
    // FBR -> removeStoredBlock -> getCachedBlocks. Only needs BM lock.
    assert namesystem.hasReadLock(FSNamesystemLockMode.GLOBAL);
    return cachedBlocks;
  }
  ...
  public final void processCacheReport(final DatanodeID datanodeID,
    ...
    try {
      ...
      namesystem.writeUnlock(FSNamesystemLockMode.GLOBAL, "processCacheReport");
    }
