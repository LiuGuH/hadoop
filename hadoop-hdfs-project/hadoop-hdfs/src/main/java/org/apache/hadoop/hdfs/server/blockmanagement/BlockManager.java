...
public class BlockManager implements BlockStatsMXBean {
  ...
  public void metaSave(PrintWriter out) {
    assert namesystem.hasReadLock(FSNamesystemLockMode.GLOBAL);
  ...
  BlockReconstructionWork scheduleReconstruction(BlockInfo block,
    ...
    if (block.isStriped()) {
      ...
      return new ErasureCodingWork(getBlockPoolId(), block, newSrcNodes,
          containingNodes, liveReplicaNodes, additionalReplRequired,
          priority, newIndices, busyIndices, excludeReconstructedIndices);
    } else {
      return new ReplicationWork(block, srcNodes,
          containingNodes, liveReplicaNodes, additionalReplRequired,
          priority);
    }
  }
  ...
  public DatanodeStorageInfo[] chooseTarget4WebHDFS(String src,
    ...
    return placementPolicies.getPolicy(CONTIGUOUS).chooseTarget(1,
  ...
  public DatanodeStorageInfo[] chooseTarget4AdditionalDatanode(String src,
    ...
    return blockplacement.chooseTarget(numAdditionalNodes, clientnode,
  ...
  public boolean processReport(final DatanodeID nodeID,
    ...
    DatanodeStorageInfo storageInfo = null;
  ...
  private void addStoredBlockImmediate(BlockInfo storedBlock, Block reported,
      DatanodeStorageInfo storageInfo)
  throws IOException {
    assert (storedBlock != null && namesystem.hasWriteLock(FSNamesystemLockMode.BM));
    if (!namesystem.isInStartupSafeMode()
  ...
  throws IOException {
    assert block != null && namesystem.hasWriteLock(FSNamesystemLockMode.BM);
  ...
  private void processMisReplicatesAsync() throws InterruptedException {
    ...
    while (namesystem.isRunning() && !Thread.currentThread().isInterrupted()) {
      int processed = 0;
      namesystem.writeLockInterruptibly(FSNamesystemLockMode.BM);
      try {
        ...
        namesystem.writeUnlock(FSNamesystemLockMode.BM, "processMisReplicatesAsync");
    ...
    try {
      while (isPopulatingReplQueues() && namesystem.isRunning()
        ...
        try {
          ...
          namesystem.writeUnlock(FSNamesystemLockMode.BM, "processMisReplicatedBlocks");
        }
      }
  ...
  /** Set storage policy id for the blocks. */
  public void setStoragePolicyId(
      final byte oldPolicyId, final byte newPolicyId, final BlockInfo b) {
    if (oldPolicyId == newPolicyId) {
      return;
    }
    b.setStoragePolicyId(newPolicyId);
  }
  ...
  private void chooseExcessRedundancies(
    ...
    // After sinking storagePolicyId into blockinfo, we can only use BM lock.
    assert namesystem.hasWriteLock(FSNamesystemLockMode.BM);
    
    if (storedBlock.isStriped()) {
      chooseExcessRedundancyStriped(nonExcess, storedBlock, delNodeHint);
    } else {
      final BlockStoragePolicy storagePolicy = storagePolicySuite.getPolicy(
          storedBlock.getStoragePolicyId());
   ...
   */
  private void chooseExcessRedundancyStriped(final Collection<DatanodeStorageInfo> nonExcess,
  ...
  /**
   * Used for NamenodeFsck#collectBlocksSummary. No need to assert holding BM lock.
   */
  public BlockInfo getStoredBlockNonThreadSafe(Block block) {
    if (!BlockIdManager.isStripedBlockID(block.getBlockId())) {
      return blocksMap.getStoredBlock(block);
    }
    if (!hasNonEcBlockUsingStripedID) {
  ...
  private class BlockReportProcessingThread extends Thread {
    ...
    public void run() {
      try {
        ...
        LOG.error("t.message is {}. code line number is {}", t.getMessage(), t.getStackTrace()[1].getLineNumber());
          ...
          try {
            ...
            if (!isInSafeMode() && ((FSNamesystem)namesystem).inActiveState() && 
              ...
              Thread.sleep(BzlDynamicConfiguration.getInstance().
                  getLong(DFS_NAMENODE_BLOCKREPORT_THREAD_SLEEP_TIME_MS,
                      DFS_NAMENODE_BLOCKREPORT_THREAD_SLEEP_TIME_MS_DEFAULT));
            }
          }
        } catch (InterruptedException e) {
