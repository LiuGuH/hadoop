...
class FSDirWriteFileOp {
  ...
  static void persistBlocks(
    ...
    assert fsd.getFSNamesystem().hasWriteLock(FSNamesystemLockMode.GLOBAL);
    ...
    try {
      ...
      if (blockType == BlockType.STRIPED) {
        ...
        fsd.updateCount(inodesInPath, 0, fileINode.getPreferredBlockSize(),
            numLocations, true);
        blockInfo = new BlockInfoStriped(block, ecPolicy, fileINode.getStoragePolicyID());
        ...
        blockInfo = new BlockInfoContiguous(block, numLocations, fileINode.getStoragePolicyID());
        blockInfo.convertToBlockUnderConstruction(
            HdfsServerConstants.BlockUCState.UNDER_CONSTRUCTION, targets);
      }
